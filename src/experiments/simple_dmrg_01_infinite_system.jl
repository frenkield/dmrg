...
#    println(size(kron(sys_enl_op[:H], I_env_enl) + kron(I_sys_enl, env_enl_op[:H])))
#    println(size(H2(sys_enl_op[:conn_Sz], sys_enl_op[:conn_Sp], env_enl_op[:conn_Sz], env_enl_op[:conn_Sp])))

#    println(sys_enl_op[:conn_Sz])
...
#    display(I_env_enl)
    ...
    superblock_hamiltonian =
        kron(sys_enl_op[:H], I_env_enl) + kron(I_sys_enl, env_enl_op[:H]) +
        H2(sys_enl_op[:conn_Sz], sys_enl_op[:conn_Sp], env_enl_op[:conn_Sz], env_enl_op[:conn_Sp])

#    println("size = $(size(superblock_hamiltonian))")


    display(superblock_hamiltonian)

    superblock_eigen = eigen(superblock_hamiltonian)
    energy = superblock_eigen.values[1]
    psi0 = superblock_eigen.vectors[:, 1]

#    println(superblock_eigen.values)
...
infinite_system_algorithm(4, 20)
